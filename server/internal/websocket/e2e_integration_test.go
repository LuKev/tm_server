...
func TestWebsocketE2E_StartGameWithSelectedMap(t *testing.T) {
	hub := NewHub()
	go hub.Run()
	...
	sendJSON(t, host, map[string]any{
		...
		"payload": map[string]any{
			...
			"maxPlayers": 2,
			"creator":    "host",
			"mapId":      string(board.MapArchipelago),
		},
	})
	created := readUntilType(t, host, "game_created", 4*time.Second)
	gameID := asString(asMap(created["payload"])["gameId"])
	...
	if got := asString(asMap(games[0])["mapId"]); got != string(board.MapArchipelago) {
		t.Fatalf("expected lobby mapId %q, got %q", board.MapArchipelago, got)
	}
	...
	state := asMap(readUntilType(t, host, "game_state_update", 4*time.Second)["payload"])
	if got := asString(state["mapId"]); got != string(board.MapArchipelago) {
		t.Fatalf("expected game state mapId %q, got %q", board.MapArchipelago, got)
	}
	mapRaw := asMap(state["map"])
	if got := asString(mapRaw["id"]); got != string(board.MapArchipelago) {
		t.Fatalf("expected map.id %q, got %q", board.MapArchipelago, got)
	}
}

...
func TestSwarmlings_StrongholdAbility(t *testing.T) {
	swarmlings := NewSwarmlings()

	ability := swarmlings.GetStrongholdAbility()
	if ability == "" {
		t.Errorf("Swarmlings should have a stronghold ability")
	}
}

func TestSwarmlings_TradingHouseUpgradeBeforeStronghold(t *testing.T) {
	swarmlings := NewSwarmlings()

	// Should not be able to use trading house upgrade before building stronghold
	if swarmlings.CanUseTradingHouseUpgrade() {
		t.Errorf("should not be able to use trading house upgrade before building stronghold")
	}

	err := swarmlings.UseTradingHouseUpgrade()
	if err == nil {
		t.Errorf("expected error when using trading house upgrade without stronghold")
	}
}

func TestSwarmlings_TradingHouseUpgradeAfterStronghold(t *testing.T) {
	swarmlings := NewSwarmlings()

	// Build stronghold
	swarmlings.BuildStronghold()

	// Should be able to use trading house upgrade
	if !swarmlings.CanUseTradingHouseUpgrade() {
		t.Errorf("should be able to use trading house upgrade after building stronghold")
	}

	// Use trading house upgrade
	err := swarmlings.UseTradingHouseUpgrade()
	if err != nil {
		t.Fatalf("failed to use trading house upgrade: %v", err)
	}

	// Should not be able to use again this Action phase
	if swarmlings.CanUseTradingHouseUpgrade() {
		t.Errorf("should not be able to use trading house upgrade twice in one Action phase")
	}

	// Try to use again (should fail)
	err = swarmlings.UseTradingHouseUpgrade()
	if err == nil {
		t.Errorf("expected error when using trading house upgrade twice")
	}
}

func TestSwarmlings_TradingHouseUpgradeReset(t *testing.T) {
	swarmlings := NewSwarmlings()
	swarmlings.BuildStronghold()
	...
	if swarmlings.CanUseTradingHouseUpgrade() {
		t.Errorf("should not be able to use trading house upgrade before reset")
	}

	// Reset for new Action phase
	swarmlings.ResetTradingHouseUpgrade()

	// Should be able to use again
	if !swarmlings.CanUseTradingHouseUpgrade() {
		t.Errorf("should be able to use trading house upgrade after reset")
	}
}

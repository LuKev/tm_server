...
- `az_selfplay -min_pass_round=N` suppresses pass actions before round `N` when non-pass actions exist. Use this as a curriculum tool for full-game self-play; leave it disabled for unconstrained policy evaluation.
...
- `az_eval -min_pass_round=N` applies the same optional full-game curriculum during arena evaluation, so a full-game promotion gate does not collapse into early-pass games.
...
The next scaling pass added a replay buffer, h512 training, and a full-game curriculum:
...
- Full-game sims=8 curriculum batch: `/tmp/tm_az_scale_next/neural_mcts_s8_50g_h512mixed_minpass6.jsonl`
  - Command shape: `-episodes=50 -max_plies=400 -sims=8 -batch_size=8 -min_pass_round=6`.
  ...
  - The `min_pass_round=6` smoke reached `8/8` games at `finalPhase=end`, proving the AZ clone replay-mode leak was fixed.
- Full-game mixed buffer: `/tmp/tm_az_scale_next/replay_buffer_mixed_fullgames.jsonl`
...
Result: candidate did not promote. It scored `23-27` (`winRate=0.46`, 95% CI `[0.322, 0.598]`) with `averagePlies=88.54`. The gate is now a real 50-game full-game gate; the model needs more data/iterations before promotion.

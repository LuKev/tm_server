...
- 2026-06-02 AlphaZero-style 1v1 engine planning:
  - Preferred next engine direction is a 1v1 Terra Mystica bot trained by self-play, using an AlphaZero-style loop with policy/value inference plus MCTS.
  - Keep the search/training rules surface attached to the live `game.Action` validation and execution path, with cloned game states for simulation, rather than building a parallel Terra Mystica rules model.
  - Avoid the earlier beam-pruned round-6 solver trust issue: do not discard legal moves only by immediate heuristic value before deeper search can see delayed payoff.
  - Current implementation doc: `docs/terra-mystica-1v1-alphazero-plan.md`; code starts under `server/internal/az`, with `server/cmd/az_selfplay` for JSONL generation, `server/cmd/az_train` for a lightweight table evaluator, `server/cmd/az_loop` for generate/train/arena/promote runs, `server/cmd/az_export` for neural-ready samples/action vocab, `server/cmd/az_train_torch` for a minimal PyTorch policy-value trainer, `server/cmd/az_infer_torch` for HTTP torch inference, and `POST /api/ai/suggest` for ranked move suggestions.
  - The current observation schema is `tm_az_board_v1`: `Position.Encode()` now returns a flat board-aware vector with shape metadata `[global feature count, board hex count, per-hex feature count]`. Self-play records include `observationSchema` / `observationShape`, and `az_export` carries that metadata plus first-row feature names into the dataset manifest and PyTorch checkpoint.
  - `POST /api/ai/execute` is the confirmation-gated bot adapter for live games. It searches the live game, chooses either the requested `actionId` or the top-ranked MCTS action, rechecks it against current legal actions, and only calls `game.Manager.ExecuteActionWithMeta` when `confirm: true` is supplied. Use `expectedRevision` and `actionRequestId` for optimistic concurrency/idempotency.
  - Live model-opponent games reuse the normal lobby/game board flow: the lobby can start a 2-player model game with a reserved `TM-AZ-<gameID>` seat, chosen human/model factions, and a strength preset through `create_and_start_model_game`. The command uses snellman setup with deterministic turn order, applies the fixed faction choices through normal `SelectFactionAction`s, disables bot turn confirmations, then `websocket.BotManager` runs model actions through `ExecuteActionWithMeta` and broadcasts ordinary `game_state_update` plus lightweight `bot_status`. The focused playable-flow smoke is `cd server && bazel test //:client_playwright_test --test_env=TM_PLAYWRIGHT_SPEC=ui-model-game.spec.ts --test_output=errors`.
  - Stronger-engine scaling runbook: `docs/terra-mystica-stronger-engine-scaling.md`. Use `training_mix` for scaled local runs; it mixes deterministic 1v1 starts with `randomized_base`, which samples base-game faction pairs, seat order, starting dwelling anchors, scoring tiles, and bonus cards.
  - `az_selfplay -metrics=<path>` writes self-play throughput/branching/timing metrics. `az_loop` runs self-play shards concurrently and its iteration reports include those metrics, dataset paths, runtime info, arena scenario counts, average plies, and win-rate confidence intervals. The torch trainer supports `--architecture=hex` for the board-shaped observation; `az_infer_torch` exposes `/evaluate_batch`, and Go MCTS uses it when `batch_size > 1` and the evaluator supports batching.
  - 100k-record local bootstrap note: a full-game scale run reached exactly 100,000 `training_mix` records with `max_plies=400` using `-sims=0`, exported them to neural JSONL, trained a 2-epoch hidden-size-256 hex checkpoint, and served it through `az_infer_torch`. Treat `-sims=0` as fast policy-prior self-play for pipeline scaling, not MCTS-improved AlphaZero data.
  - Terminal-game quality fix: the AZ action surface now prunes ordinary main-turn transform-only actions, and scenario bootstrap clears `BonusCards.PlayerHasCard` after assigning starting bonus cards so pass actions are legal. A 20-game neural `sims=8`, `max_plies=400` self-play run produced 2,065 records, 0 truncations, and all games reached round-6 end state; the follow-up h256 checkpoint did not promote in an 8-game smoke (`3/8` vs the 100k bootstrap checkpoint).
  - `az_loop` maintains `ratings.json` with lightweight Elo-style model ratings across iterations. Treat it as a trend signal across retained checkpoints; promotion decisions should still use arena games, scenario mix, and confidence intervals. `-promote_min_games` and `-promote_ci95_lower_bound` make that gate executable; reports include a structured `promotion` object with blocking reasons.
  - Use `az_eval` for direct retained-model comparisons without running a train loop. It accepts table model paths or HTTP evaluator URLs for candidate and baseline and writes the same arena metrics and promotion-decision shape as `az_loop`.
  - Replay-derived midgame sampling path: `az_replay_seeds` emits snapshot seed JSONL from one replay text file or a replay fixture directory (`-input_dir`, `-pattern`, `-max_per_replay`), and self-play can consume it with `-scenario=snapshots:/path/to/seeds.jsonl`. Use `-summary` to audit seed coverage by source, round, phase, player count, root faction, and faction presence before training. Filters (`-phase`, `-root_faction`, `-player_count`, `-min_round`, `-max_round`) build narrower curriculum pools; summary `skippedByReason` shows when filters are too tight. Parsed snapshot seeds are normalized before AZ use so player IDs, confirmation options, and pending-state maps are compatible with live legal-action generation. The snapshot parser accepts generated terrain names such as plural `Plains` in map overrides.
  - Neural evaluator wiring: use `TM_AZ_MODEL_URL=http://127.0.0.1:9097/evaluate` for the live server API, `-model_url` for `az_selfplay`, and `-base_model_url` for `az_loop` before a promoted `best_model.json` exists. The torch scripts preload stdlib `cmd` because Bazel runfiles expose the repo's top-level `cmd/` directory, which otherwise shadows Python's stdlib module during torch imports.
...
- 2026-04-24 Fire & Ice BGA replay testing:
  ...
  - BGA table `670716682` is a Fire & Ice map game with Nomads, Riverwalkers, Dragonlords, and Giants. Missing bonus tiles: `BON-SHIP`, `BON-6C`, `BON-WP`. Config lives at `server/internal/replay/testdata/bga_670716682_config.yaml`.
  - `670716682` inferred scoring tiles are `SCORE5,SCORE4,SCORE3,SCORE7,SCORE8,SCORE6`; the strict replay reaches exact final scores: kezilu/Nomads 159, felipebart/Dragonlords 144, haligh/Riverwalkers 143, Barnawal/Giants 140.
  ...
  - Riverwalkers terrain unlocks are resolved as a pending priest choice at the moment a priest is gained, including income and the priest power action. The player chooses either to take the priest into resources or unlock a terrain, and the UI prompt shows each terrain's coin cost; these unlocks are no longer exposed as free conversion buttons.
  - Fire & Ice extra-cost rule for Riverwalkers and volcano factions: Ice factions, Riverwalkers, Dragonlords, Acolytes, and Firewalkers do not make their selected starting terrain an opponent Home terrain, so unlocking/transforming that color does not cost extra. Shapeshifters do choose a Home terrain, so their selected Home terrain still charges the extra cost.
...
- 2026-06-04 AZ full-game scaling pass:
  ...
  - Do not hide pass actions for training. A no-pass-hiding smoke after the replay-mode fix reached `8/8` games at round 6/end, confirming immediate passes should still advance through rounds normally.
  - AZ legal-action enumeration now blocks ordinary main-turn actions for players whose `HasPassed` flag is set; pending reactions still come from the pending-action path. This fixed a worker-smoke case where a passed player could receive a later upgrade action during pending resolution.
  ...
  - Historical local full-game artifacts live under `/tmp/tm_az_scale_next`: `neural_mcts_s8_50g_h512mixed_minpass6.jsonl` (`4272` records, `49/50` terminal round-6 games), `replay_buffer_mixed_fullgames.jsonl` (`26337` records), and `tm_az_policy_value_h512_mixed_fullgames.pt`. Regenerate the scaled batch with normal pass actions before treating it as preferred training data.
  ...
  - Current clean normal-pass artifacts live under `/tmp/tm_az_scale_clean`: `neural_mcts_s8_50g_h512_normalpass_w4.jsonl` (`5045` records, `46/50` terminal games, `4` truncations, `17.28` records/sec), `replay_buffer_mixed_normalpass.jsonl` (`27110` records), export `replay_buffer_mixed_normalpass_export` (`6169` actions), and checkpoint `tm_az_policy_value_h512_mixed_normalpass.pt` (`hidden_size=512`, final loss `1.7084`).
  - `az_selfplay`, `az_eval`, and `az_loop` now expose worker/progress knobs: `az_selfplay -workers=N -progress`, `az_eval -workers=N -progress`, and `az_loop -selfplay_workers=N -arena_workers=N -progress`. Loop self-play metrics now use wall-clock elapsed time for merged shard throughput while preserving summed legal/search/apply timing.
  - Clean h512 arena gate vs the historical h512 checkpoint: candidate scored `27-23` over 50 games (`winRate=0.54`, 95% CI `[0.402,0.678]`) but did not promote because the CI lower bound missed `0.45`. Four-worker arena runtime was about `6.46` minutes versus the earlier single-worker gate at about `12.5` minutes.
  - After worker parallelism, the clean 50-game neural sims=8 run produced `17.28` records/sec. Remaining performance targets are scenario-level truncation diagnostics, quieter torch HTTP logging, and in-process/ONNX inference to reduce HTTP JSON overhead.
  - Follow-up performance knobs implemented: `az_selfplay -compact_records`, `-reuse_tree`, `-global_batch_size`, `az_loop` equivalents, and `az_infer_torch --torch-threads/--torch-interop-threads`; torch HTTP access logs are now off by default. On a 12-game neural h512 benchmark (`training_mix`, `max_plies=400`, `sims=8`, `batch_size=8`, seed `3101`), baseline was `11.35` records/sec; compact/tree/global-batch was `11.37` records/sec with output shrinking from `27MB` to `15MB`; 8 workers was slower at `10.86` records/sec; torch single-thread tuning was flat at `11.38` records/sec. Treat compact records as an I/O/storage win, not a proven throughput win.
  - Meaningful follow-up speedup: torch evaluator now supports binary float32 endpoints and Go HTTP inference uses them with JSON fallback; MCTS batch search deduplicates repeated leaf nodes before evaluator calls. On the same 12-game h512 benchmark, binary transport alone reached `11.58` records/sec, binary plus MCTS leaf dedupe reached `12.21`, and binary plus dedupe plus compact/tree/global-batch knobs reached `12.46` records/sec. Treat `12.2-12.4` records/sec as the current realistic local range for that benchmark.
  - Promotion cycle result: `tm_az_policy_value_h512_candidate.pt` under `/tmp/tm_az_promotion_20260604` was trained from the existing clean normal-pass buffer plus a fresh 60-game neural MCTS batch (`5070` records, `58/60` terminal, `17.98` records/sec). The 100-game arena gate vs `/tmp/tm_az_scale_clean/tm_az_policy_value_h512_mixed_normalpass.pt` scored `63-35-2` for the candidate (`winRate=0.64`, 95% CI `[0.54592,0.73408]`) and passed `minGames=100`, `minWinRate=0.5`, `minCi95LowerBound=0.45`. During this run, experimental subtree reuse selected a stale town action; `Tree.Advance` now refreshes to a new root until state identity checks make subtree reuse safe.
  - Full ordered-matchup scale pass: `matrix:base_ordered` covers all `168` legal ordered base-faction 1v1 starts (same-faction and same-home-terrain pairs excluded), and `az_selfplay`, `az_eval`, and `az_loop` now default to `max_plies=500`. A neural h512 run on `/tmp/tm_az_matrix_20260604` generated `75,208` new records from `840` matrix games (`795/840` terminal, `45` truncated, `16.83` records/sec), mixed them with the prior promoted buffer into `107,388` records, exported `8,829` actions, and trained `tm_az_policy_value_h512_matrix.pt` for `5` epochs (final loss `0.5751`). The all-matchup 168-game arena vs `/tmp/tm_az_promotion_20260604/tm_az_policy_value_h512_candidate.pt` scored `82-81-5` for the matrix candidate (`winRate=0.50298`, 95% CI `[0.42737,0.57858]`) and did not promote because the CI lower bound missed `0.45`. Arena coverage was exactly one game per ordered matchup; `157/168` games ended normally and `11` truncated at 500 plies, all in action phase and all in unordered matchups involving `Mermaids`. Treat Mermaids-paired long-tail action loops as the next quality target before simply raising plies again.
  - Mermaids truncation root cause found after the matrix pass: AZ was generating `special_mermaids_town` candidates from normal build/action target hexes, while `NewMermaidsRiverTownAction` expects a river hex, and the game validator did not enforce that the target terrain was river. The Mermaids connect executor could also accept repeating the same already-delayed river town as a successful no-op; because Mermaids connect suppresses turn advance and does not track normal special-action usage, MCTS could waste plies in a legal-looking action loop. The fix is to validate river targets in `SpecialAction.validateMermaidsRiverTown`, reject repeated no-op delayed connects, and have AZ enumerate actual river hexes for Mermaids connect. Regression tests live in `special_actions_test.go` and `actions_test.go`; a post-fix `matchup:Halflings:Mermaids` smoke (`8` games, `max_plies=500`, `sims=8`) reached `8/8` terminal round-6 games with `0` truncations and `9.44` records/sec.
  - Post-Mermaids fixed promotion: `/tmp/tm_az_post_mermaids_fix/selfplay_matrix.jsonl` generated `105,460` records from `1,680` ordered-matrix games, all terminal with `0` truncations; Mermaids oversampling added `19,991` records from `320` games; a V3-fixed model then added `21,382` ordered-matrix records from `336` games and `13,837` targeted weak-matchup records from `216` games, again all terminal with `0` truncations. `az_train_torch` now supports checkpoint initialization, bounded streaming shuffle, and action-vocab transfer so new action rows can be initialized conservatively while preserving matching policy rows from the incumbent. The promoted fixed h512 checkpoint is `/tmp/tm_az_post_mermaids_fix/tm_az_policy_value_h512_v3_targeted_repair1.pt`: after conservative low-LR training on the `192,850`-record targeted repair buffer, the 168-game `matrix:base_ordered` arena vs `/tmp/tm_az_promotion_20260604/tm_az_policy_value_h512_candidate.pt` scored `89-79` (`winRate=0.5297619`, 95% CI `[0.4542873,0.6052365]`) and passed `minGames=168`, `minWinRate=0.5`, `minCi95LowerBound=0.45`, with `0` truncated arena games.
  - 5k-scale self-play pass: `/tmp/tm_az_scale_5k_20260605` generated about `5,000` complete ordered-matrix games by sharding self-play into three outputs (`187,848`, `66,204`, and `66,095` records; `320,147` new records total), all observed games terminal at round 6/end with `0` truncations before interrupting near the target. Mixing those shards with the post-Mermaids targeted repair buffer produced `/tmp/tm_az_scale_5k_20260605/replay_buffer_5k_augmented.jsonl` with `512,997` records and an export with `11,185` actions. A first h512 transfer candidate at LR `2e-5` did not promote (`78-88-2`, winRate `0.470238`, 95% CI `[0.394764,0.545713]`, `0` truncations). A more conservative h512 transfer candidate at LR `1e-5` did promote: `/tmp/tm_az_scale_5k_20260605/tm_az_policy_value_h512_5k_candidate_lr1e5.pt` scored `91-70-7` over the 168-game `matrix:base_ordered` arena vs `/tmp/tm_az_post_mermaids_fix/promoted_h512_fixed.pt` (`winRate=0.5625`, 95% CI `[0.487484,0.637516]`, average margin `0.04018`, `168/168` terminal round-6/end, `0` truncations). The promoted symlink is `/tmp/tm_az_scale_5k_20260605/promoted_h512_5k.pt`.

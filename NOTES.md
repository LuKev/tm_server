...
- 2026-06-02 AlphaZero-style 1v1 engine planning:
  ...
  - Live model-opponent games reuse the normal lobby/game board flow: the lobby can create a 2-player model game with a reserved `TM-AZ-<gameID>` seat, chosen human/model factions, and a strength preset. Start uses snellman setup with deterministic turn order, applies the fixed faction choices through normal `SelectFactionAction`s, disables bot turn confirmations, then `websocket.BotManager` runs model actions through `ExecuteActionWithMeta` and broadcasts ordinary `game_state_update` plus lightweight `bot_status`.

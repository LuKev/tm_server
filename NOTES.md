...
- 2026-06-02 AlphaZero-style 1v1 engine planning:
  ...
  - Replay-derived midgame sampling path: `az_replay_seeds` emits snapshot seed JSONL from one replay text file or a replay fixture directory (`-input_dir`, `-pattern`, `-max_per_replay`), and self-play can consume it with `-scenario=snapshots:/path/to/seeds.jsonl`. Parsed snapshot seeds are normalized before AZ use so player IDs, confirmation options, and pending-state maps are compatible with live legal-action generation. The snapshot parser accepts generated terrain names such as plural `Plains` in map overrides.
